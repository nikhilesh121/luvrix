...
export const getAllBlogs = async (statusFilter = null, fetchAll = false) => {
  ...
  // If fetchAll is true (admin mode), return all blogs; otherwise limit to 50
  return fetchAll ? blogs : blogs.slice(0, 50);
};
...
export const getGiveaway = async (idOrSlug) => {
  if (!idOrSlug) return null;
  // Try by slug first
  const q = query(collection(db, "giveaways"), where("slug", "==", idOrSlug), limit(1));
  const snapshot = await getDocs(q);
  if (!snapshot.empty) return serializeDoc(snapshot.docs[0]);
  // Try by ID
  const docRef = doc(db, "giveaways", idOrSlug);
  const docSnap = await getDoc(docRef);
  return serializeDoc(docSnap);
};

export const getGiveawayBySlug = async (slug) => {
  return getGiveaway(slug);
};
...
export const updateWatchtime = async (userId, seconds) => {
  if (!userId || !seconds || seconds <= 0) return false;
  try {
    const docRef = doc(db, "watchtime", userId);
    const docSnap = await getDoc(docRef);
    ...
    console.error("Error updating watchtime:", error);
    return false;
  }
};
...
export const getWatchtime = async (userId) => {
  ...
  try {
    const docRef = doc(db, "watchtime", userId);
    const docSnap = await getDoc(docRef);
    if (!docSnap.exists()) return { userId, seconds: 0 };
    return { id: docSnap.id, ...docSnap.data() };
  } catch (error) {
    console.error("Error getting watchtime:", error);
    return null;
  }
};
...
export const hideUserPosts = async (userId) => {
  ...
  for (const docSnap of snapshot.docs) {
    await updateDoc(doc(db, "blogs", docSnap.id), { status: "hidden" });
  }
  return true;
};
...
export const unhideUserPosts = async (userId) => {
  ...
  const snapshot = await getDocs(q);
  for (const docSnap of snapshot.docs) {
    await updateDoc(doc(db, "blogs", docSnap.id), { status: "approved" });
  }
  return true;
};
...
export const incrementFreePostsUsed = async (userId) => {
  const docRef = doc(db, "users", userId);
  await updateDoc(docRef, { freePostsUsed: increment(1) });
  return true;
};

export const decrementExtraPosts = async (userId) => {
  const docRef = doc(db, "users", userId);
  await updateDoc(docRef, { extraPosts: increment(-1) });
  return true;
};
...
export const getTrendingTopics = async () => {
  ...
  let topics = serializeDocs(snapshot);
  topics.sort((a, b) => (b.order || 0) - (a.order || 0));
  return topics;
};
...
export const updatePayment = async (txnId, data) => {
  const q = query(collection(db, "payments"), where("txnId", "==", txnId), limit(1));
  const snapshot = await getDocs(q);
...
export const initiatePayment = async (data) => {
  // For static export, payment initiation should happen on client-side
  // This is a placeholder that stores the payment intent
...
export const generateBlogDraft = async (data) => {
  // Placeholder for AI draft generation - this would need external API
  return { success: false, error: "AI draft generation not available in static mode" };
};
...
export const rejectBlog = async (blogId, reason = "") => {
  const docRef = doc(db, "blogs", blogId);
  await updateDoc(docRef, { status: "rejected", rejectionReason: reason, rejectedAt: new Date() });
  return true;
};
...
export const getBlogDrafts = async (userId) => {
  const q = userId 
    ? query(collection(db, "drafts"), where("authorId", "==", userId))
    : query(collection(db, "drafts"));
  const snapshot = await getDocs(q);
  let drafts = serializeDocs(snapshot);
  drafts.sort((a, b) => new Date(b.updatedAt || b.createdAt || 0) - new Date(a.updatedAt || a.createdAt || 0));
  return drafts;
};
...
export const publishBlogDraft = async (draftId, data = {}) => {
  const draft = await getBlogDraft(draftId);
  if (!draft) return { success: false, error: "Draft not found" };
  
  const blogId = await createBlog({ ...draft, ...data, status: "pending" });
...
export const updateManga = async (mangaId, data) => {
  ...
  let docRef;
  const q = query(collection(db, "manga"), where("slug", "==", mangaId), limit(1));
  const snapshot = await getDocs(q);
  if (!snapshot.empty) {
    docRef = doc(db, "manga", snapshot.docs[0].id);
  } else {
    docRef = doc(db, "manga", mangaId);
  }
  await updateDoc(docRef, { ...data, updatedAt: new Date() });
  return true;
};
...
export const deleteManga = async (mangaId) => {
  ...
  const snapshot = await getDocs(q);
  if (!snapshot.empty) {
    await deleteDoc(doc(db, "manga", snapshot.docs[0].id));
  } else {
...
export const listGiveaways = async (status = null) => {
  const snapshot = await getDocs(collection(db, "giveaways"));
  let giveaways = serializeDocs(snapshot);
  if (status) {
    giveaways = giveaways.filter(g => g.status === status);
  }
  giveaways.sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
  return giveaways;
};
...
export const updateGiveawayApi = async (giveawayId, data) => {
  const docRef = doc(db, "giveaways", giveawayId);
  await updateDoc(docRef, { ...data, updatedAt: new Date() });
  return true;
};

export const deleteGiveaway = async (giveawayId) => {
  const docRef = doc(db, "giveaways", giveawayId);
  await deleteDoc(docRef);
  return true;
};

export const getGiveawayTasks = async (giveawayId) => {
  const giveaway = await getGiveaway(giveawayId);
  return giveaway?.tasks || [];
};

export const getGiveawayParticipants = async (giveawayId) => {
  const q = query(collection(db, "giveaway_participants"), where("giveawayId", "==", giveawayId));
  const snapshot = await getDocs(q);
  return serializeDocs(snapshot);
};

export const addGiveawayTask = async (giveawayId, taskData) => {
  const giveaway = await getGiveaway(giveawayId);
  const tasks = giveaway?.tasks || [];
  const newTask = { ...taskData, id: `task_${Date.now()}` };
  tasks.push(newTask);
  await updateGiveawayApi(giveawayId, { tasks });
  return newTask;
};

export const editGiveawayTask = async (giveawayId, taskId, taskData) => {
  const giveaway = await getGiveaway(giveawayId);
  const tasks = (giveaway?.tasks || []).map(t => t.id === taskId ? { ...t, ...taskData } : t);
  await updateGiveawayApi(giveawayId, { tasks });
  return true;
};

export const removeGiveawayTask = async (giveawayId, taskId) => {
  const giveaway = await getGiveaway(giveawayId);
  const tasks = (giveaway?.tasks || []).filter(t => t.id !== taskId);
  await updateGiveawayApi(giveawayId, { tasks });
  return true;
};

export const selectGiveawayWinner = async (giveawayId, winnerData) => {
  await updateGiveawayApi(giveawayId, { winner: winnerData, status: "completed" });
  return true;
};
...
export const getAllSubscribers = async () => {
  ...
  let subscribers = serializeDocs(snapshot);
  subscribers.sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
  return subscribers;
};
...
export const getGiveawayWinnerInfo = async (giveawayId) => {
  const giveaway = await getGiveaway(giveawayId);
  return giveaway?.winner || null;
};

// ============================================
// GIVEAWAY PARTICIPANT FUNCTIONS
// ============================================

export const getGiveawayParticipantCount = async (giveawayId) => {
  const q = query(collection(db, "giveaway_participants"), where("giveawayId", "==", giveawayId));
  const snapshot = await getDocs(q);
  return snapshot.size;
};

export const joinGiveaway = async (userId, giveawayId) => {
  const participantId = `${userId}_${giveawayId}`;
  const docRef = doc(db, "giveaway_participants", participantId);
...
export const getMyGiveawayStatus = async (userId, giveawayId) => {
  const participantId = `${userId}_${giveawayId}`;
  const docRef = doc(db, "giveaway_participants", participantId);
  const docSnap = await getDoc(docRef);
...
export const completeGiveawayTask = async (userId, giveawayId, taskId) => {
  const participantId = `${userId}_${giveawayId}`;
  const docRef = doc(db, "giveaway_participants", participantId);
  const docSnap = await getDoc(docRef);
  if (!docSnap.exists()) {
  ...
  if (!completedTasks.includes(taskId)) {
    ...
    await updateDoc(docRef, { completedTasks, updatedAt: new Date() });
  }
  return { success: true, completedTasks };
};

export const getGiveawayInterest = async (giveawayId, userId = null) => {
  const q = query(collection(db, "giveaway_interest"), where("giveawayId", "==", giveawayId));
  const snapshot = await getDocs(q);
  const count = snapshot.size;
  let interested = false;
  if (userId) {
    interested = snapshot.docs.some(d => d.data().userId === userId);
  }
  return { count, interested };
};

export const toggleGiveawayInterest = async (userId, giveawayId) => {
  const interestId = `${userId}_${giveawayId}`;
  const docRef = doc(db, "giveaway_interest", interestId);
  const existing = await getDoc(docRef);
  if (existing.exists()) {
    await deleteDoc(docRef);
    const newCount = await getGiveawayInterest(giveawayId);
    return { interested: false, count: newCount.count };
  } else {
    await setDoc(docRef, { userId, giveawayId, createdAt: new Date() });
    const newCount = await getGiveawayInterest(giveawayId);
    return { interested: true, count: newCount.count };
  }
};

export const getGiveawaySupportData = async (giveawayId) => {
  const q = query(collection(db, "giveaway_support"), where("giveawayId", "==", giveawayId));
  const snapshot = await getDocs(q);
  const supporters = serializeDocs(snapshot);
  const total = supporters.reduce((sum, s) => sum + (s.amount || 0), 0);
  return { total, count: supporters.length, supporters };
};

export const submitGiveawayShipping = async (userId, giveawayId, shippingData) => {
  const participantId = `${userId}_${giveawayId}`;
  const docRef = doc(db, "giveaway_participants", participantId);
  await updateDoc(docRef, { shipping: shippingData, shippingSubmittedAt: new Date() });
  return { success: true };
};

export const getGiveawayShipping = async (userId, giveawayId) => {
  const participantId = `${userId}_${giveawayId}`;
  const docRef = doc(db, "giveaway_participants", participantId);
  const docSnap = await getDoc(docRef);
  if (!docSnap.exists()) return null;
  return { shipping: docSnap.data().shipping || null };
};
