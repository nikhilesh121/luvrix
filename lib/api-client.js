...
// API Base URL - uses backend server URL for static export on Hostinger
...
async function getCSRFToken() {
  ...
  const csrfUrl = API_URL ? `${API_URL}/api/csrf-token` : "/api/csrf-token";
  _csrfPromise = fetch(csrfUrl, { credentials: API_URL ? "include" : "same-origin" })
    .then(r => r.json())
...
export const editGiveawayTask = (id, taskId, data) =>
  fetchAPI(`/giveaways/${id}/tasks`, { method: "PUT", body: JSON.stringify({ taskId, ...data }) });
