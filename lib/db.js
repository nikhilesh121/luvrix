...
const toObjectId = (id) => {
  ...
  return id; // Return as-is for custom IDs like Firebase UIDs
};
...
export const getBlog = async (id) => {
  ...
  if (!blog) {
    ...
    if (objId !== id) {
      const blogById = await db.collection('blogs').findOne({ _id: objId });
      return blogById ? { id: blogById._id.toString(), ...blogById } : null;
    }
  }
  return blog ? { id: blog._id.toString(), ...blog } : null;
};
...
export const getAllBlogs = async (statusFilter = null, excludeBlockedUsers = true, limitCount = 50) => {
  ...
  return blogs.map(b => ({ id: b._id.toString(), ...b }));
};
...
export const getManga = async (id) => {
  ...
  return manga ? { id: manga._id.toString(), ...manga } : null;
};
...
export const getMangaBySlug = async (slug) => {
  ...
  return { id: manga._id.toString(), ...manga };
};
...
export const getAllManga = async () => {
  ...
  const manga = await db.collection('manga')
    ...
    .sort({ createdAt: -1 })
    .toArray();
  return manga.map(m => ({ id: m._id.toString(), ...m }));
};
...
export const getAllPayments = async () => {
  ...
  const payments = await db.collection('payments')
    ...
    .sort({ createdAt: -1 })
    .toArray();
  return payments.map(p => ({ id: p._id.toString(), ...p }));
};
...
export const getAllRewards = async () => {
  ...
  const rewards = await db.collection('rewards')
    ...
    .sort({ createdAt: -1 })
    .toArray();
  return rewards.map(r => ({ id: r._id.toString(), ...r }));
};
...
export const getUserFavorites = async (userId) => {
  ...
  const favorites = await db.collection('favorites')
    .find({ userId })
    .toArray();
  return favorites.map(f => ({ id: f._id, ...f }));
};
...
export const getSitemapUrls = async () => {
  ...
  return urls.map(u => ({ id: u._id.toString(), ...u }));
};
...
export const getAllSubscribers = async () => {
  ...
  const subscribers = await db.collection('subscribers')
    ...
    .sort({ subscribedAt: -1 })
    .toArray();
  return subscribers.map(s => ({ id: s._id.toString(), ...s }));
};
...
export const unfollowUser = async (followerId, followingId) => {
  ...
  await db.collection('follows').deleteOne({ _id: followId });
...
export const likeBlog = async (userId, blogId) => {
  ...
  await db.collection('blogLikes').updateOne(
    { _id: likeId },
    { 
      $set: { 
        userId,
...
export const unlikeBlog = async (userId, blogId) => {
  ...
  await db.collection('blogLikes').deleteOne({ _id: likeId });
...
export const getUserLibraries = async (userId, includePrivate = false) => {
  ...
  return libraries.map(l => ({ id: l._id.toString(), ...l }));
};
...
export const getLibrary = async (libraryId) => {
  ...
  return library ? { id: library._id.toString(), ...library } : null;
};
...
export const getBlogDrafts = async (status = "draft") => {
  ...
  const drafts = await db.collection('blogDrafts')
    ...
    .sort({ createdAt: -1 })
    .toArray();
  return drafts.map(d => ({ id: d._id.toString(), ...d }));
};
...
export const getBlogDraft = async (draftId) => {
  ...
  return draft ? { id: draft._id.toString(), ...draft } : null;
};
