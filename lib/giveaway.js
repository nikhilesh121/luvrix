...
export function checkEligibility(participant, giveaway, tasks) {
  ...
  // 2. Check points threshold if set (applies to both random and task_gated modes)
  if ((giveaway.requiredPoints || 0) > 0 && participant.points < giveaway.requiredPoints) {
    return false;
  }
...
export async function createGiveaway(data) {
  ...
  const giveaway = {
    ...
    sponsors: Array.isArray(data.sponsors) ? data.sponsors.filter(s => s.bannerUrl) : [],
...
export async function updateTask(taskId, data) {
  ...
  if (data.points !== undefined) updateFields.points = Number(data.points) || 1;
  ...
  await db.collection("giveaway_tasks").updateOne({ _id: objectId }, { $set: updateFields });
  const updated = await db.collection("giveaway_tasks").findOne({ _id: objectId });
...
export async function joinGiveaway(giveawayId, userId) {
  ...
  // In random mode with no required tasks and no points threshold, user is immediately eligible
  const tasks = await getTasksForGiveaway(giveawayId);
  const hasRequiredTasks = tasks.filter(t => t.required).length > 0;
  const hasPointsThreshold = (giveaway.requiredPoints || 0) > 0;
  if (!hasRequiredTasks && !hasPointsThreshold) {
...
export async function completeTask(giveawayId, userId, taskId) {
  ...
  // Default minimum timers per task type (seconds) — enforced server-side
  ...
  // Use admin-set timer if present, otherwise use default for the task type
  const timerDuration = (task.metadata?.timerDuration > 0)
    ? task.metadata.timerDuration
    : (DEFAULT_TIMERS[task.type] || 0);

  if (timerDuration > 0) {
    const taskStart = await db.collection("giveaway_task_starts").findOne({
      userId, taskId, giveawayId,
    });
    if (!taskStart) throw new Error("You must visit the task link first");
    const elapsed = (Date.now() - new Date(taskStart.startedAt).getTime()) / 1000;
    if (elapsed < timerDuration - 2) { // 2s grace for network latency
      throw new Error(`Please wait ${Math.ceil(timerDuration - elapsed)} more seconds`);
    }
  }
...
export async function processInvite(inviteCode, newUserId) {
  ...
  // Prevent duplicate invite — check if this user was already referred
  const alreadyReferred = await db.collection("giveaway_invites").findOne({
    giveawayId: inviter.giveawayId,
    referredUserId: newUserId,
  });
  if (alreadyReferred) throw new Error("This user was already referred");

  // Record the invite
  await db.collection("giveaway_invites").insertOne({
    giveawayId: inviter.giveawayId,
...
export async function getGiveawaySupporters(giveawayId, { isAdmin = false } = {}) {
  ...
  // Aggregate by userId: sum amounts, use latest donation's anonymous/name settings
  const aggregated = {};
  supports.forEach(s => {
    const uid = s.userId;
    if (!aggregated[uid]) {
    ...
    aggregated[uid].totalAmount += s.amount;
    aggregated[uid].donationCount += 1;
    // Use the latest donation's preferences
    if (s.createdAt > aggregated[uid].latestDate) {
      aggregated[uid].latestDate = s.createdAt;
      aggregated[uid].donorName = s.donorName;
      aggregated[uid].isAnonymous = s.isAnonymous;
    }
  });

  // Sort by total amount descending
  const sortedUsers = Object.values(aggregated).sort((a, b) => b.totalAmount - a.totalAmount);

  return sortedUsers.map(agg => {
    const user = userMap[agg.userId];
    const publicName = agg.isAnonymous ? "Anonymous" : (agg.donorName || user?.displayName || user?.name || "Anonymous");
    const result = {
      id: agg.userId,
      userId: agg.userId,
      userName: publicName,
      userPhoto: agg.isAnonymous ? null : (user?.photoURL || null),
      amount: agg.totalAmount,
      donationCount: agg.donationCount,
      isAnonymous: !!agg.isAnonymous,
    };

    // Admin gets full details always
    if (isAdmin) {
      result.donorName = agg.donorName || user?.displayName || user?.name || "Unknown";
      result.donorEmail = agg.donorEmail || user?.email || "";
      result.userEmail = user?.email || "";
      result.userName = agg.donorName || user?.displayName || user?.name || "Unknown";
