...
/**
 * Ensure URL path ends with trailing slash (matches trailingSlash: true in next.config.js)
 */
function ensureTrailingSlash(url) {
  if (!url) return url;
  // Don't touch full URLs with query params or hash
...
async function submitIndexNow(url) {
  ...
  try {
    const fullUrl = ensureTrailingSlash(url.startsWith("http") ? url : `${SITE_URL}${url}`);
...
async function logIndexRequest(url, type, results) {
  try {
    ...
    await db.collection("index_requests").insertOne({
      url,
      fullUrl: ensureTrailingSlash(url.startsWith("http") ? url : `${SITE_URL}${url}`),
      type,
      results,
      createdAt: new Date(),
    });
...
/**
 * Notify search engines that a URL has been permanently removed.
 * Uses Google Indexing API URL_DELETED + IndexNow re-ping.
 */
export async function autoDeindex(url, type = "page") {
  console.log(`[auto-index] De-indexing ${type}: ${url}`);

  const results = [];

  // IndexNow: re-submit the URL — search engines will see the 410 and de-index
  if (INDEXNOW_KEY) {
    try {
      const fullUrl = ensureTrailingSlash(url.startsWith("http") ? url : `${SITE_URL}${url}`);
      const res = await fetch("https://api.indexnow.org/indexnow", {
      ...
      results.push({ success: res.ok || res.status === 202, engine: "indexnow_deindex", status: res.status });
    } catch (err) {
      results.push({ success: false, engine: "indexnow_deindex", error: err.message });
    }
  }

  // Google Indexing API: send URL_DELETED
  const credsJson = process.env.GOOGLE_INDEXING_CREDENTIALS;
  if (credsJson) {
    try {
      let JWT;
      try {
        ...
        results.push({ success: false, engine: "google_deindex", error: "google-auth-library not installed" });
      }
      if (JWT) {
        const creds = JSON.parse(credsJson);
        const jwtClient = new JWT({
        ...
        const res = await fetch("https://indexing.googleapis.com/v3/urlNotifications:publish", {
          ...
          body: JSON.stringify({ url: fullUrl, type: "URL_DELETED" }),
        });
        const data = await res.json();
        results.push({ success: res.ok, engine: "google_deindex", status: res.status, data });
      }
    } catch (err) {
      results.push({ success: false, engine: "google_deindex", error: err.message });
    }
  }

  await logIndexRequest(url, `${type}_deleted`, results);
  return { url, type: `${type}_deleted`, results };
}
