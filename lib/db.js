...
export const createManga = async (data) => {
  ...
  if (data.slug) {
    ...
    // Note: chapter URLs are NOT added to sitemap (noindex architecture)
...
export const updateManga = async (id, data) => {
  ...
  // Auto-index manga page when chapters are added or manga is updated
  if (data.slug && data.totalChapters && oldManga) {
    ...
    if (newChapters > oldChapters) {
      // Note: chapter URLs are NOT added to sitemap (noindex architecture)
      // Instead, re-index the manga detail page so Google sees updated chapter count
      autoIndex(`/manga/${data.slug}`, 'manga').catch(err =>
